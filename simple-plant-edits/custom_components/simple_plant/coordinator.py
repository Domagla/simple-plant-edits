...
class SimplePlantCoordinator(DataUpdateCoordinator[dict]):
    ...
    async def async_mark_as_watered_toggle(self) -> None:
        ...
        if "last_watered" in data:
            last_watered = self._parse_stored_datetime(data["last_watered"])
        if "_old_last_watered" in data:
            old_last_watered = self._parse_stored_datetime(data["_old_last_watered"])
    ...
    def _parse_stored_datetime(self, value: str) -> datetime:
        """Parse storage datetime; treat naive values as local for legacy data."""
        parsed = datetime.fromisoformat(value)
        if parsed.tzinfo is None:
            local_tz = ZoneInfo(self.hass.config.time_zone)
            return as_utc(parsed.replace(tzinfo=local_tz))
